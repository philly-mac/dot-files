...
{
  if (( $UID == 0 )) ; then
    printf "%s" "\[\033[0m\]\\u\[\033[0m\]@\[\033[0m\]\\h\[\033[0m\]:\w\[\033[0m\] "
  else
    printf "%s" "\[\033[0m\]\\u\[\033[0m\]@\[\033[0m\]\\h\[\033[0m\]:\w\[\033[0m\] "
  fi
}
...
ps1_rvm()
{
  command -v rvm-prompt >/dev/null 2>&1 && printf "%s" "\[\033[32m\]$(rvm-prompt)\[\033[0m\]"
}
...
ps1_update()
{
  ...
  # if (( notime > 0 )) ; then
  #   PS1="$(ps1_titlebar)$(ps1_identity)$(ps1_git)$(ps1_rvm)${separator}\[\033[31m\]${prompt_char}\[\033[0m\] "
  # else
  #   PS1="$(ps1_titlebar)\D{%H:%M:%S} $(ps1_identity)$(ps1_git)$(ps1_rvm)${separator}[\033[31m\]${prompt_char}\[\033[0m\] "
  # fi
